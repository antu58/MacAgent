...
# Fallback: stop old MLX server still occupying the target port.
if command -v lsof >/dev/null 2>&1; then
  PORT_PIDS="$(lsof -t -iTCP:"$PORT" -sTCP:LISTEN -n -P 2>/dev/null | sort -u || true)"
  if [[ -n "${PORT_PIDS:-}" ]]; then
    for pid in $PORT_PIDS; do
      cmd="$(ps -p "$pid" -o command= 2>/dev/null || true)"
      if [[ "$cmd" == *"mlx_lm.server"* ]]; then
        echo "Found old MLX process on port $PORT (PID=$pid), stopping it..."
        stop_pid_gracefully "$pid"
      else
        echo "Port $PORT is occupied by non-MLX process (PID=$pid)." >&2
