...
本项目用于在本地运行 **Qwen3.5 9B 多模态版 (MLX 8bit)** 模型服务，提供 OpenAI 兼容接口：

- 服务地址：`http://127.0.0.1:18083/v1`
- 当前包含 `9B` 图文多模态服务能力
...
4. 网络：首次初始化需要下载 `mlx-vlm[torch]` 依赖；模型仍复用当前 `mlx-community/Qwen3.5-9B-MLX-8bit`
5. 磁盘：至少预留 `15GB+`（模型缓存、虚拟环境、日志）

## 快速开始

```bash
cd /Users/zhangfeng/Desktop/Project/Apple/MacAgent/Qwen_9_B
chmod +x scripts/*.sh
...
2. `run/`：运行时产物目录
   - `qwen35-9b-mlx.log`：服务日志
   - `qwen35-9b-mlx.pid`：后台进程 PID
...
- `MLX_SERVICE_HOME=/Users/zhangfeng/.mlx-qwen35-9b`
- `VENV_DIR=/Users/zhangfeng/.mlx-qwen35-9b/venv`
- `HF_HOME=/Users/zhangfeng/.mlx-qwen35-9b/hf`
- `HUGGINGFACE_HUB_CACHE=/Users/zhangfeng/.mlx-qwen35-9b/hf/hub`
- `MAX_KV_SIZE=131072`
...
```bash
./scripts/test_image_chat.sh http://127.0.0.1:18083/v1/chat/completions https://images.cocodataset.org/val2017/000000039769.jpg
./scripts/test_image_chat.sh http://127.0.0.1:18083/v1/chat/completions /Users/zhangfeng/Desktop/截屏2026-03-19\ 12.45.04.png
```
...
- `http://host.docker.internal:18083/v1`

## 当前范围与后续补充

当前范围：

- 支持 `Qwen3.5-9B-MLX-8bit (MLX)` 本地多模态服务
